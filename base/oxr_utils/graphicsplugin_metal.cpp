...
struct MetalGraphicsPlugin : public IGraphicsPlugin {
  ...
  void InitializeResources() {
    ...
    m_library =
        NS::TransferPtr(m_device->newLibrary(NS::String::string(shaderSrc, UTF8StringEncoding), nullptr, &pError));
    if (!m_library) {
    ...
    m_fragmentFunction =
        NS::TransferPtr(m_library->newFunction(NS::String::string("fragmentMain", UTF8StringEncoding)));
  ...
  int64_t SelectColorSwapchainFormat(const std::vector<int64_t>& runtimeFormats) const override {
    ...
    constexpr MTL::PixelFormat SupportedColorSwapchainFormats[] = {MTL::PixelFormatRGBA8Unorm_sRGB,
  ...
  void RenderView(const XrCompositionLayerProjectionView& layerView, const XrSwapchainImageBaseHeader* swapchainImage,
    ...
    if (!swapchainContext.m_cubeMatricesBuffer ||
        swapchainContext.m_cubeMatricesBuffer->length() != matricesBufferLength) {
    ...
    swapchainContext.m_cubeMatricesBuffer->didModifyRange(
        NS::Range::Make(0, swapchainContext.m_cubeMatricesBuffer->length()));
