...
struct RenderTarget {
  ...
  void Create(const VulkanDebugObjectNamer& namer, VkDevice device, VkImage aColorImage, VkImage aDepthImage,
...
struct DepthBuffer {
  ...
  void TransitionLayout(CmdBuffer* cmdBuffer, VkImageLayout newLayout) {
    ...
    vkCmdPipelineBarrier(cmdBuffer->buf, VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT, VK_PIPELINE_STAGE_ALL_GRAPHICS_BIT, 0, 0,
...
// adjust the window size and show at InitDevice time
  ...
  hWnd = CreateWindowW(wc.lpszClassName, L"hello_xr (Vulkan)", WS_OVERLAPPEDWINDOW | WS_VISIBLE, CW_USEDEFAULT,
                       CW_USEDEFAULT, rect.right - rect.left, rect.bottom - rect.top, 0, 0, hInst, 0);
  assert(hWnd != NULL);
...
#include "frag.spv"
  ...
  std::vector<XrSwapchainImageBaseHeader*> AllocateSwapchainImageStructs(
    ...
    // doesn't need to know the type/size. Keep the buffer alive by adding it into the list of buffers.
    ...
    std::vector<XrSwapchainImageBaseHeader*> bases =
        swapchainImageContext.Create(m_namer, m_vkDevice, &m_memAllocator, capacity, swapchainCreateInfo,
...
struct VulkanGraphicsPluginLegacy : public VulkanGraphicsPlugin {
  ...
  virtual XrResult CreateVulkanInstanceKHR(XrInstance instance, const XrVulkanInstanceCreateInfoKHR* createInfo,
    ...
    CHECK_XRCMD(pfnGetVulkanInstanceExtensionsKHR(instance, createInfo->systemId, extensionNamesSize,
