...
#error Platform not supported
  ...
  int64_t SelectColorSwapchainFormat(const std::vector<int64_t>& runtimeFormats) const override {
    ...
    constexpr int64_t SupportedColorSwapchainFormats[] = {
        GL_RGB10_A2,
        GL_RGBA16F,
        // The two below should only be used as a fallback, as they are linear color formats without enough bits for
        // color depth, thus leading to banding.
        GL_RGBA8,
        GL_RGBA8_SNORM,
    };
  ...
  void RenderView(const XrCompositionLayerProjectionView& layerView, const XrSwapchainImageBaseHeader* swapchainImage,
    ...
    for (const Cube& cube : cubes) {
      ...
      // Draw the cube.
      glDrawElements(GL_TRIANGLES, static_cast<GLsizei>(ArraySize(Geometry::c_cubeIndices)), GL_UNSIGNED_SHORT,
                     nullptr);
    }
