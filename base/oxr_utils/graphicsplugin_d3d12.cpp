...
namespace {
void InitializeD3D12DeviceForAdapter(IDXGIAdapter1* adapter, D3D_FEATURE_LEVEL minimumFeatureLevel,
                                     ID3D12Device** device) {
#if !defined(NDEBUG)
  ...
  CHECK_HRCMD(
      D3D12CreateDevice(adapter, minimumFeatureLevel, __uuidof(ID3D12Device), reinterpret_cast<void**>(device)));
}
...
ComPtr<ID3D12Resource> CreateBuffer(ID3D12Device* d3d12Device, uint32_t size, D3D12_HEAP_TYPE heapType) {
  ...
  CHECK_HRCMD(d3d12Device->CreateCommittedResource(&heapProp, D3D12_HEAP_FLAG_NONE, &buffDesc, d3d12ResourceState,
...
struct D3D12GraphicsPlugin : public IGraphicsPlugin {
  ...
  void InitializeResources() {
    ...
    CHECK_HRCMD(m_device->CreateRootSignature(0, rootSignatureBlob->GetBufferPointer(),
    ...
    CHECK_HRCMD(m_device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, initializeContext.GetCommandAllocator(),
    ...
    m_cubeIndexBuffer = CreateBuffer(m_device.Get(), sizeof(Geometry::c_cubeIndices), D3D12_HEAP_TYPE_DEFAULT);
    {
      ...
      cmdList->CopyBufferRegion(m_cubeIndexBuffer.Get(), 0, cubeIndexBufferUpload.Get(), 0,
  ...
  std::vector<XrSwapchainImageBaseHeader*> AllocateSwapchainImageStructs(
    ...
    // xrEnumerateSwapchainImages). Return back an array of pointers to each swapchain image struct so the consumer
  ...
  ID3D12PipelineState* GetOrCreatePipelineState(DXGI_FORMAT swapchainFormat) {
    ...
    CHECK_HRCMD(
        m_device->CreateGraphicsPipelineState(&pipelineStateDesc, __uuidof(ID3D12PipelineState),
  ...
  void RenderView(const XrCompositionLayerProjectionView& layerView, const XrSwapchainImageBaseHeader* swapchainImage,
    ...
    ComPtr<ID3D12GraphicsCommandList> cmdList;
    CHECK_HRCMD(m_device->CreateCommandList(0, D3D12_COMMAND_LIST_TYPE_DIRECT, swapchainContext.GetCommandAllocator(),
                                            nullptr, __uuidof(ID3D12GraphicsCommandList),
                                            reinterpret_cast<void**>(cmdList.ReleaseAndGetAddressOf())));
    ...
    constexpr uint32_t cubeCBufferSize =
        AlignTo<D3D12_CONSTANT_BUFFER_DATA_PLACEMENT_ALIGNMENT>(sizeof(ModelConstantBuffer));
